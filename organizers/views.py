...
class ApplicationDetailView(TabsViewMixin, IsOrganizerMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if(self.can_vote()):
            context['apps_left_to_vote'] = models.HackerApplication.objects.exclude(vote__user_id=self.request.user.id)\
            .filter(status=APP_PENDING, submission_date__lte=timezone.now() - timedelta(hours=2)).count()
