...
class ApplicationFilter(django_filters.FilterSet):
    ...
    status = django_filters.MultipleChoiceFilter(
        "status", label="Status", choices=STATUS, widget=forms.CheckboxSelectMultiple
    )
    ...
    class Meta:
        ...
        fields = (
            ["search", "status", "under_age", "online"]
            if getattr(settings, "HYBRID_HACKATHON", False)
            else ["search", "status", "under_age"]
        )


class DubiousApplicationFilter(django_filters.FilterSet):
    search = django_filters.CharFilter(method="search_filter", label="Search")
    contacted = django_filters.ChoiceFilter(
        "contacted",
        label="Hacker contacted?",
        null_label=None,
        empty_label=None,
...
class AdminTeamListTable(tables.Table):
    selected = tables.CheckBoxColumn(
        accessor="user__team__team_code", verbose_name="Select"
    )
